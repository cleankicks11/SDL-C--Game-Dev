...
void LTimer::pause()
{
	//IF the timer is running and isn't slready paused
	if( mStarted && !mPaused )
	{
		//Pause the timer
		mPaused = true;
...
bool loadMedia()
{
	...
	else
	{
		...
		{
			cout << "Unable to render start/stop prompt texture!\n" << endl;
			success = false;
		}
		...
		{
			cout << "Unable to render pause/unpause prompt texture\n" << endl;
			success = false;
		}
	}
	...
	else
	{
		...
		else
		{
			...
			while( !quit )
			{
				...
				timeText.str( "" );
				timeText << "Seconds since since start time" << ( timer.getTicks() / 1000.f ) ;
